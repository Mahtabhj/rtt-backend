...
export function ImpactAssessmentForm({
  ...
  saveRegulatoryFrameworkImpactAssessmentAnswers,
  selectedQuestionsId,
          ...
          <>
            <Modal.Body className="overlay overlay-block cursor-default">
              ...
              <Form className="form form-label-right">
                {/* {relatedRegulation && relatedRegulation.map((regulation) => {
                    return (
                      <> */}
                        {/* <h3>Regulation: {regulation.name}</h3> */}

                        {values.questions.map((question, index) => {
                          const answer = impactAssessmentAnswers && impactAssessmentAnswers.find(
                              (ans) =>
                                ans.question === question.id
                                // && ans.regulation === regulation.id
                            );
                          const dateCreated = getFullDate(toDate(parseISO(answer?.created)));
                          const dateModified = getFullDate(toDate(parseISO(answer?.modified)));
                          const inChargePerson = userList.find(
                            (user) => user.id === question.in_charge
                          ).username;
                          const answeredBy = answer?.answered_by && userList.find(
                              (user) => user.id === answer?.answered_by
                            ).username;
                                  ...
                                  <div className="col-lg-6">
                                    <Field
                                      ...
                                      onChange={(e) => {
                                        ...
                                        ans.question = question.id;
                                        // ans.regulation = regulation.id;
                                        ans.regulatory_framework = regulatoryFrameworkId;
                                  ...
                                  <div className="col-lg-6">
                                    ...
                                    {answer?.answered_by && (
                                      <p>
                                        ...
                                        {answeredBy}, {dateCreated}
                                      </p>
                                    )}
                                    {answer?.answered_by && (
                                      <p>
                                        <strong>Last edited by:</strong>{" "}
                                        {answeredBy} <strong>on</strong>{" "}
                                        {dateModified}
                                      </p>
                                    )}
                                  </div>
