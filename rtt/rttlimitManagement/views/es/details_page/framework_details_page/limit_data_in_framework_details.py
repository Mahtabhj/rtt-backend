...
class LimitInFrameworkDetails(APIView):
    ...
    def post(self, request, framework_id):
        """
        doc: https://chemycal.atlassian.net/browse/RTT-675
        """
        try:
            if not framework_id:
            ...
            for limit in regulation_substance_limit_qs:
                limit_list.append({
                    'has_exemption': ExemptionExistenceCheck().has_exemption_data(
                        framework_id, is_regulation=False, substance_id=limit.substance.id),
                    'substance': {
                    ...
                    'additional_attributes': AdditionalAttributesDataService().get_additional_attributes_data(
                        limit.id, framework_id, is_regulation=False)
                })
            response = {
